            ...
            compile: function(tElement, tAttrs) {
              ...
              return function link(scope, element, attrs, $select, transcludeFn) {
                ...
                scope.$watch('$select.open', function(isOpen) {
                  if (isOpen) {
                    $select.onDropdown(attrs.ondropdown);
                  }
                });
  ...
  uis.controller('uiSelectCtrl',
      ['$scope', '$element', '$timeout', '$filter', 'uisRepeatParser', 'uiSelectMinErr', 'uiSelectConfig', '$parse',
        function($scope, $element, $timeout, $filter, RepeatParser, uiSelectMinErr, uiSelectConfig, $parse) {
          ...
          var _dropdownDelayPromise;
          ctrl.onDropdown = function(dropdownAttr) {
            if (dropdownAttr !== undefined) {

              // Debounce
              // See https://github.com/angular-ui/bootstrap/blob/0.10.0/src/typeahead/typeahead.js#L155
              // FYI AngularStrap typeahead does not have debouncing: https://github.com/mgcrea/angular-strap/blob/v2.0.0-rc.4/src/typeahead/typeahead.js#L177
              if (_dropdownDelayPromise) {
                $timeout.cancel(_dropdownDelayPromise);
              }
              _dropdownDelayPromise = $timeout(function() {
                $scope.$eval(dropdownAttr);
              }, ctrl.refreshDelay);
            }
          };
