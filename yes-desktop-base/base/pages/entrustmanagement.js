define(['plugins/base/pages/entrustmanagement.config',
        'plugins/base/pages/entrustmanagement.detail'],
    function (config) {
        angular.module('app').controller('base.entrustmanagement',
            function ($scope, $rootScope, utils, path, getBaseView, settings,
                      $timeout, dialog, toastr) {

                var scope = getBaseView($scope, 'plugins/base/pages/entrustmanagement.detail.html'); //虚拟继承基础的view.
                    ...
                    scope.action = {
                        ...
                        save: function (event, form) {
                            ...
                            if (form.$valid) {
                                ...
                                utils.ajax({
                                    method: method,
                                    url: "base/entrustmanagement" + (data.uid ? ("/" + data.uid) : ""),
                                    mockUrl: "plugins/base/data/entrustmanagement.save.json",
                                    data: data
                                }).then(function (res) {
                                    scope.load();
                                    scope.unloadDetail();
                            ...
                            if (rows.length) {
                                dialog.confirm('确定删除该条记录?').then(function () {
                                    ...
                                    angular.forEach(rows, function (row) {
                                        utils.ajax({
                                            method: "DELETE",
                                            url: 'base/entrustmanagement/' + row.uid,
                                            mockUrl: 'plugins/base/data/entrustmanagement.del.json'
                                        }).then(function (res) {
                                            loading++;
                ...
                scope.load = function () {
                    ...
                    utils.ajax({
                        method: 'GET',
                        url: 'base/entrustmanagement',
                        mockUrl: "plugins/base/data/entrustmanagements.json",
                        params: data
                    }).then(function (res) {
                        var data = res.data;
                        scope.gridOptions.columnDefs = utils.gridDefine(scope.headers, "base/entrustmanagement");
