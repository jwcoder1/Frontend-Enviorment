define(['plugins/base/pages/mqueue.config'/*,
        'plugins/base/pages/mqueue.detail'*/],
    function (config) {
        angular.module('app').controller('base.mqueue',
            function ($scope, utils, path, getBaseView, settings, $translate,
                      $timeout, dialog, toastr) {

                var scope = getBaseView($scope, 'plugins/base/pages/mqueue.detail.html'); //虚拟继承基础的view.
                ...
                scope.binds = function () {
                    ...
                    scope.action = {
                        add: function (event, data) {
                            dialog.open({
                                template: 'plugins/base/pages/mqueue.detail.html',
                                scope: scope
                            });
                        },
                        save: function (event, form) {
                            ...
                            if (form.$valid) {
                                ...
                                utils.ajax({
                                    method: method,
                                    url: "base/mqueue",
                                    mockUrl: "plugins/base/data/mqueue.save.json",
                                    data: data
                                }).then(function (res) {
                                    scope.load();
                                });
                            ...
                            if (rows.length) {
                                dialog.confirm('确定删除该条记录?').then(function () {
                                    angular.forEach(rows, function (row) {
                                        utils.ajax({
                                            method: "DELETE",
                                            url: 'base/mqueue/' + row.uid,
                                            mockUrl: 'plugins/base/data/mqueue.del.json'
                                        }).then(function (res) {
                                            scope.load();
                                        });
                                    });
                ...
                scope.load = function () {
                    ...
                    utils.ajax({
                        method: 'GET',
                        url: 'base/mqueue',
                        mockUrl: "plugins/base/data/mqueues.json",
                        params: data
                    }).then(function (res) {
                        var data = res.data;
                        scope.gridOptions.columnDefs = utils.gridDefine(scope.headers, "base/mqueue");
