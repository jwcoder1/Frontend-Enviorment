define(['plugins/base/pages/mqueuecfg.config',
        'plugins/base/pages/mqueuecfg.detail'],
    function (config) {
        angular.module('app').controller('base.mqueuecfg',
            function ($scope, utils, path, getBaseView, settings,
                      $timeout, dialog, toastr) {

                var scope = getBaseView($scope, 'plugins/base/pages/mqueuecfg.detail.html'); //虚拟继承基础的view.
                    ...
                    scope.action = {
                        ...
                        save: function (event, form) {
                            ...
                            if (form.$valid) {
                                ...
                                utils.ajax({
                                    method: method,
                                    url: "base/mqueuecfg" + (data.uid ? ("/" + data.uid) : ""),
                                    mockUrl: "plugins/base/data/mqueuecfg.save.json",
                                    data: data
                                }).then(function (res) {
                                    scope.load();
                                    scope.unloadDetail();
                            ...
                            if (rows.length) {
                                dialog.confirm('确定删除该条记录?').then(function () {
                                    ...
                                    angular.forEach(rows, function (row) {
                                        utils.ajax({
                                            method: "DELETE",
                                            url: 'base/mqueuecfg/' + row.uid,
                                            mockUrl: 'plugins/base/data/mqueuecfg.del.json'
                                        }).then(function (res) {
                                            loading++;
                ...
                scope.load = function () {
                    ...
                    utils.ajax({
                        method: 'GET',
                        url: 'base/mqueuecfg',
                        mockUrl: "plugins/base/data/mqueuecfgs.json",
                        params: data
                    }).then(function (res) {
                        var data = res.data;
                        scope.gridOptions.columnDefs = utils.gridDefine(scope.headers, "base/mqueuecfg");
