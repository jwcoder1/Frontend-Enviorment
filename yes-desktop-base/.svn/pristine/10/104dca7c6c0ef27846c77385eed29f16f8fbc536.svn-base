define(['plugins/base/pages/logmodule.config',
        'plugins/base/pages/logmodule.detail'],
    function (config) {
        angular.module('app').controller('base.logmodule',
            function ($scope, utils, path, getBaseView, settings,
                      $timeout, dialog, toastr) {

                var scope = getBaseView($scope, 'plugins/base/pages/logmodule.detail.html'); //虚拟继承基础的view.
                    ...
                    scope.action = {
                        ...
                        save: function (event, form) {
                            ...
                            if (form.$valid) {
                                ...
                                utils.ajax({
                                    method: method,
                                    url: "base/logmodule" + (data.uid ? ("/" + data.uid) : ""),
                                    mockUrl: "plugins/base/data/logmodule.save.json",
                                    data: data
                                }).then(function (res) {
                                    scope.load();
                                    scope.unloadDetail();
                            ...
                            if (rows.length) {
                                dialog.confirm('确定删除该条记录?').then(function () {
                                    ...
                                    angular.forEach(rows, function (row) {
                                        utils.ajax({
                                            method: "DELETE",
                                            url: 'base/logmodule/' + row.uid,
                                            mockUrl: 'plugins/base/data/logmodule.del.json'
                                        }).then(function (res) {
                                            loading++;
                ...
                scope.load = function () {
                    ...
                    utils.ajax({
                        method: 'GET',
                        url: 'base/logmodule',
                        mockUrl: "plugins/base/data/logmodules.json",
                        params: data
                    }).then(function (res) {
                        var data = res.data;
                        scope.gridOptions.columnDefs = utils.gridDefine(scope.headers, "base/logmodule");
