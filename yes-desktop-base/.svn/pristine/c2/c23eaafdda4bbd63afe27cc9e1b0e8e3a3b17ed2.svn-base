define(['plugins/base/pages/menu.config',
        'plugins/base/pages/menu.detail'],
    function (config) {
        angular.module('app').controller('base.menu',
            function ($scope, utils, path, getBaseView, settings,
                      $timeout, dialog, toastr) {

                var scope = getBaseView($scope, 'plugins/base/pages/menu.detail.html'); //虚拟继承基础的view.
                    ...
                    scope.action = {
                        ...
                        save: function (event, form) {
                            ...
                            if (form.$valid) {
                                ...
                                utils.ajax({
                                    method: method,
                                    url: "base/menu" + (data.uid ? ("/" + data.uid) : ""),
                                    mockUrl: "plugins/base/data/menu.save.json",
                                    data: data
                                }).then(function (res) {
                                    scope.load();
                                    scope.unloadDetail();
                            ...
                            if (rows.length) {
                                dialog.confirm('确定删除该条记录?').then(function () {
                                    ...
                                    angular.forEach(rows, function (row) {
                                        utils.ajax({
                                            method: "DELETE",
                                            url: 'base/menu/' + row.uid,
                                            mockUrl: 'plugins/base/data/menu.del.json'
                                        }).then(function (res) {
                                            loading++;
                ...
                scope.load = function () {
                    ...
                    utils.ajax({
                        method: 'GET',
                        url: 'base/menu',
                        mockUrl: "plugins/base/data/menus.json",
                        params: data
                    }).then(function (res) {
                        var data = res.data;
                        scope.gridOptions.columnDefs = utils.gridDefine(scope.headers, "base/menu");
